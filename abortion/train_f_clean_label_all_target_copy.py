...
def train(netC, optimizerC, schedulerC, train_dl, tf_writer, epoch, opt, normalize, trigger, filter, trigger_total):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        inputs_bd = fft_attack(opt, inputs_bd_origin, inject, opt.f_ratio, num_bd, filter)
        
        # do augmentation on total inputs
        total_inputs = torch.cat([inputs_bd, inputs[num_bd:, :, :, :]])
        ...
        if batch_idx == len(train_dl) - 2:
            # # print(bs_target)
            residual = inputs[:num_bd, :, :, :] - inputs_bd 
            total_inputs_bd = denormalizer(total_inputs[:num_bd, :, :, :])
            batch_img = torch.cat([inputs[:num_bd, :, :, :], inputs_bd, residual, total_inputs_bd], dim=2)
...
def eval(
    ...
    for batch_idx, (inputs, targets) in enumerate(test_dl):
        with torch.no_grad():
            ...
            inject_index = targets.fill_(opt.target_label)
            inputs_bd = fft_attack(opt, inputs, trigger_total[inject_index, :, :, :], opt.f_ratio, bs, filter)

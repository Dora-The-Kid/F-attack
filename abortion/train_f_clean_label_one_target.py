...
def train(netC, optimizerC, schedulerC, train_dl, tf_writer, epoch, opt, normalize, trigger, filter, trigger_total):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        if num_bd != 0:
            inject = trigger_total[opt.target_label].repeat(num_bd, 1, 1, 1)
            target_index = (torch.eq(targets, opt.target_label))
            inputs_bd_origin = inputs[target_index, :, :, :]

            inputs_bd = fft_attack(opt, inputs_bd_origin[:num_bd, :, :, :], inject, opt.f_ratio, num_bd, filter)
        ...
        total_clean_correct += torch.sum(
            torch.argmax(total_preds[num_bd :], dim=1) == targets[(num_bd) :]
        )
        ...
        if batch_idx == len(train_dl) - 2 and num_bd != 0:
            # print(bs_target)
            residual = inputs_bd_origin[:num_bd, :, :, :] - inputs_bd             # no normalize
...
def main():
    ...
    if opt.continue_training:
        ...
        trigger_total = torch.zeros(10, 1, 3, 32, 32)

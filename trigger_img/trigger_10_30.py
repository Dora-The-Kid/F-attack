...
def triger_insert(IMG):
    ...
    print(np.median(filter))
    print(np.max(filter), np.min(filter))
    # filter[filter>np.median(filter)] = 1
    print(np.median(filter))
    print(np.max(filter), np.min(filter))
    # filter = cv2.GaussianBlur(filter, ksize=(11, 11), sigmaX=10)
    # print(filter)
    ...
    #insert trigger
    IMG_with_trigger_fft = IMG_FFT+trigger_final_fft*50
...
if __name__ == '__main__':
    ...
    cv2.imwrite('123456789.jpeg',np.abs(a))
    # plt.figure()
